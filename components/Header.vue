<template>
  ...
  <option value="fastasy">🦄 Fantasy</option> 
      ...
      <div class="inline-flex md:hidden" x-data="{ open: false }">
        <button class="flex-none px-2 btn btn-link btn-sm" x-on:click="open = true">
          <svg
        ...
        <div class="absolute top-0 left-0 right-0 z-50 flex flex-col p-2 pb-4 m-2 space-y-3 bg-white rounded shadow" x-show.transition="open" x-on:click.away="open = false" x-cloak>
          <button class="self-end flex-none px-2 ml-2 btn btn-link btn-icon" x-on:click="open = false">
            <svg
              xmlns="http://www.w3.org/2000/svg"
              width="24"
              height="24"
